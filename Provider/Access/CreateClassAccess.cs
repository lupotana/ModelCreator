...
{   
    public class CreateClassAccess
    {
        public DataTable table = new DataTable();
        private string keyCode = string.Empty;
        public CreateClassAccess() { }
        ...
        public bool BuildTemplate(DataTable dt, Provider provider)
        {
            ...
            template = ManageTemplate.GetTemplate("CrudTemplateAccess.ico");
            ...
            if (!(ManageTemplate.WriteTemplate(Session.Folder, template, string.Concat(tableName, "_Base"), null))) return false;
            {
                ...
                {
                    template = ManageTemplate.GetTemplate("CrudTemplatePlusAccess.ico");
        ...
        private void SetAllKeys(ref string declarationParametersKeys, ref string listParameterKeys, ref string parametersKeysWithOutType, ref string isNullKeys, ref string parameters)
        {
            foreach (KeyValuePair<string, KeyColumn> kvp in Session.Keys)
            {
                ...
                if (!kvp.Key.StartsWith("VERSION_"))
                {
                    ...
                    parameters = string.Format("{0}\t\t\tOleDbParameter parameter{1} = new OleDbParameter(\"{1}\", DbType.{2}); \n", parameters, GetPublic(kvp.Key), dbType);
                    parameters = string.Concat(parameters, "\t\t\tDataHelperAccess.ManageParameter(ref parameter", GetPublic(kvp.Key), ", ", GetPublic(kvp.Key), "); \n");
                }
            }
                ...
                if (!dr["Name"].ToString().StartsWith("VERSION_"))
                {
                    ...
                    if (Session.TableKey == dr["Name"].ToString())
                    {
                        if (provider == Provider.SqlBase)
                        {
                            ...
                            parameters = string.Concat(parameters, "\t\t\t\telse DataHelperAccess.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", ", GetPublic(dr["Name"].ToString()), "); \n");
                        }
                        else
                    ...
                    else
                    {
                        if (dr["Name"].ToString().Equals("N_UPD_VER"))
                        {
                            ...
                            parameters = string.Concat(parameters, "\t\t\t\telse DataHelperAccess.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", 1); \n");
                        }
                        else
                            parameters = string.Concat(parameters, "\t\t\t\tDataHelperAccess.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", ", GetPublic(dr["Name"].ToString()), "); \n");
                    }
        ...
        private Dictionary<string, KeyColumn> GetKeys(string tableName)
        {
            ...
            connection.Open();
            DataTable dt = connection.GetOleDbSchemaTable(OleDbSchemaGuid.Primary_Keys, null);
            connection.Close();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                DataTable dtColumn = GetDatatableAccess.GetSchema(tableName, column.Name);
