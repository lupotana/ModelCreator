...
{   
    public class CreateClassPostGres
    {
        public DataTable table = new DataTable();
        private string keyCode = string.Empty;
        public CreateClassPostGres() { }
        ...
        public bool BuildTemplate(DataTable dt, Provider provider)
        {
            ...
            Session.Keys = GetKeys(tableName);
            string keyType = "int";
            string isAutomaticKey = "false";
            string typeKey = string.Empty;

            //// DETEC IF IS AUTO INCREMENT KEY CLASS
            ...
            //                case "SMALLINT":
            //                case "FLOAT":
            //                case "INTEGER": Session.TableKey = kvp.Key; keyType = GetDatatableSqlBase.GetSqlBaseType(string.Empty, kvp.Value.Type, 0, false); isAutomaticKey = "true"; break;
            //            }
            //        }
            //    }
            //}
            ...
            template = ManageTemplate.GetTemplate("CrudTemplatePostGres.ico");
            ...
            //Dictionary<string, Dictionary<string, string>> dicionaryIndex = new Dictionary<string, Dictionary<string, string>>();
            //DataTable dtIndex = GetDatatableSqlBase.GetIndex(tableName);
            //string indexName = string.Empty;
            //Dictionary<string, string> dictionaryFields = new Dictionary<string, string>();
            //foreach (DataRow drIndex in dtIndex.Rows)
            //{
            //    if ((!indexName.Equals(drIndex["SYSKEYS.IXNAME"].ToString())))
            //    {
            //        if (!indexName.Equals(string.Empty))
            //        {
            //            dicionaryIndex.Add(indexName, dictionaryFields);
            //            dictionaryFields = new Dictionary<string, string>();
            //        }
            //    }
            //    indexName = drIndex["SYSKEYS.IXNAME"].ToString();
            //    foreach (DataRow drColumn in dt.Rows)
            //        if (drColumn["FieldName"].ToString().Equals(drIndex["SYSKEYS.COLNAME"].ToString()))
            //            dictionaryFields.Add(drColumn["FieldName"].ToString(), drColumn["Type"].ToString());
            //}
            string indexReplace = string.Empty;
            //dicionaryIndex.Add(indexName, dictionaryFields);
            //string templateIndex = string.Empty;
            //foreach (KeyValuePair<string, Dictionary<string, string>> kvpIndex in dicionaryIndex)
            //{
            //    templateIndex = ManageTemplate.GetTemplate("Snippet/CheckExist.ico");
            //    templateIndex = templateIndex.Replace("***DeclareIndex***", GetDeclareParametersIndex(kvpIndex.Value));
            //    templateIndex = templateIndex.Replace("***ParameterKeys***", GetDeclareParametersIndexKeys(kvpIndex.Value, dt));
            //    templateIndex = templateIndex.Replace("***ListParameterKeys***", GetDeclareParametersIndexList(kvpIndex.Value));
            //    templateIndex = templateIndex.Replace("#KeysIndex#", GetDeclareParametersIndexKeyWhere(kvpIndex.Value));
            //    templateIndex = templateIndex.Replace("#TableName#", tableName);

            //    indexReplace = string.Concat(indexReplace, " ", templateIndex, "\n");
            //}
            template = template.Replace("***CheckExist***", indexReplace);
            ...
            //template = template.Replace("***InsertSql***", GetInsertSql(dt, tableName));
            //template = template.Replace("#UpdateSql#", GetUpdateSql(dt, tableName));
            //template = template.Replace("#DeleteSql#", GetDeleteSql(dt, tableName));
            ...
            bool isValid = false;
            if (!(ManageTemplate.WriteTemplate(Session.Folder, template, string.Concat(tableName, "_Base"), null))) return false;
            {
                #region Class Plus

                if (Session.CreateCustomClass)
                {
                    template = ManageTemplate.GetTemplate("CrudTemplatePlusPostGres.ico");
        ...
        public bool BuildDbShared()
        {
            string template = ManageTemplate.GetTemplate("DbShared.ico");
            template = template.Replace("***NameSpace***", Session.NamespaceDataLayer);
            if (ManageTemplate.WriteTemplate(Session.Folder, template, "DbShared", null)) return true;
            else return false;
        }

        public bool BuildSqlHelperGupta()
        {
            string template = ManageTemplate.GetTemplate("SqlHelperGupta.ico");
            template = template.Replace("***NameSpace***", Session.NamespaceDataLayer);
            if (ManageTemplate.WriteTemplate(Session.Folder, template, "SqlHelperGupta", null)) return true;
            else return false;
        }
        ...
        private void GetSql(DataTable dt, string tableName, ref string queryInsert, ref string queryUpdate, ref string queryDelete)
        {
            queryInsert = "INSERT INTO {3}.{0} ({1}) VALUES ({2})";
            string fieldNameInsert = string.Empty;
            string fieldValueInsert = string.Empty;
            queryUpdate = "UPDATE {3}.{0} SET {1} WHERE {2}";
            string fieldNameUpdate = string.Empty;
            string whereConditionUpdate = string.Empty;
            queryDelete = "DELETE FROM {2}.{0} WHERE ({1})";
            string whereConditionDelete = string.Empty;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    //INSERT
                    fieldNameInsert = string.Concat(fieldNameInsert, dr["Name"].ToString(), ",");
                    fieldValueInsert = string.Concat(fieldValueInsert, ":", dr["Name"].ToString(), ",");

                    //UPDATE
                    if (!Session.Keys.ContainsKey(dr["Name"].ToString()))
                        fieldNameUpdate = string.Concat(fieldNameUpdate, dr["Name"].ToString(), " = :", dr["Name"].ToString(), ",");
                    else whereConditionUpdate = string.Concat(whereConditionUpdate, dr["Name"].ToString(), " = :", dr["Name"].ToString(), " AND ");
                    //DELETE
                    if (Session.Keys.ContainsKey(dr["Name"].ToString()))
                        whereConditionDelete = string.Concat(whereConditionDelete, dr["Name"].ToString(), " = :", dr["Name"].ToString(), " AND ");
                }
            }
            //INSERT
            queryInsert = string.Format(queryInsert, tableName, fieldNameInsert.Substring(0, fieldNameInsert.Length - 1), fieldValueInsert.Substring(0, fieldValueInsert.Length - 1), Session.Owner);
            //UPDATE
            if (fieldNameUpdate.Equals(string.Empty)) fieldNameUpdate = whereConditionUpdate;
            if (whereConditionUpdate.Equals(string.Empty)) whereConditionUpdate = fieldNameUpdate;
            queryUpdate = string.Format(queryUpdate, tableName, fieldNameUpdate.Substring(0, fieldNameUpdate.Length - 1).Trim(), whereConditionUpdate.Substring(0, whereConditionUpdate.Length - 4).Trim(), Session.Owner);
            //DELETE
            queryDelete = string.Format(queryDelete, tableName, whereConditionDelete.Substring(0, whereConditionDelete.Length - 4).Trim(), Session.Owner);
        }

        private string GetInsertSql(DataTable dt, string tableName)
        {
            string query = "INSERT INTO {0} ({1}) VALUES ({3}.{2})";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (!dr["Name"].ToString().StartsWith("VERSION_"))
                {
                    fieldName = string.Concat(fieldName, dr["Name"].ToString(), ",");
                    fieldValue = string.Concat(fieldValue, ":", dr["Name"].ToString(), ",");
                }
            }

            query = string.Format(query, tableName, fieldName.Substring(0, fieldName.Length - 1), fieldValue.Substring(0, fieldValue.Length - 1), Session.Owner);

            return query;
        }

        private string GetInsertSqlCounter(DataTable dt, string tableName)
        {
            string query = "INSERT INTO {0} ({1}) VALUES ({3}.{2})";
            string fieldName = string.Empty;
            string fieldValue = string.Empty;
        ...
        private string GetUpdateSql(DataTable dt, string tableName)
        {
            string query = "UPDATE {0} SET ({1}) WHERE {3}.{2}";
            string fieldName = string.Empty;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    if (!Session.Keys.ContainsKey(dr["Name"].ToString()))
                        fieldName = string.Concat(fieldName, dr["Name"].ToString(), " = :", dr["Name"].ToString(), ",");
                    else whereCondition = string.Concat(whereCondition, dr["Name"].ToString(), " = :", dr["Name"].ToString(), " AND ");
                }
            }

            if (fieldName.Equals(string.Empty)) fieldName = whereCondition;
            if (whereCondition.Equals(string.Empty)) whereCondition = fieldName;
            query = string.Format(query, tableName, fieldName.Substring(0, fieldName.Length - 1).Trim(), whereCondition.Substring(0, whereCondition.Length - 4).Trim(), Session.Owner);
            return query;
        }

        private string GetDeleteSql(DataTable dt, string tableName)
        {
            string query = "DELETE FROM {0} WHERE ({2}.{1})";
            string whereCondition = string.Empty;

            foreach (DataRow dr in dt.Rows)
            {
                if (Session.Keys.ContainsKey(dr["Name"].ToString()))
                    whereCondition = string.Concat(whereCondition, dr["Name"].ToString(), " = :", dr["Name"].ToString(), " AND ");
            }

            query = string.Format(query, tableName, whereCondition.Substring(0, whereCondition.Length - 4).Trim(), Session.Owner);
            return query;
        }

        private string GetViewSql(DataTable dt, string tableName, bool withOrder)
        {
            string query = string.Empty;
            if (withOrder)
                query = "SELECT {0} FROM {2}.{1} {3}";
            else query = "SELECT {0} FROM {2}.{1} {4} {3}";
            string fieldName = string.Empty;
            ...
            if (withOrder)
                query = string.Format(query, fieldName.Substring(0, fieldName.Length - 1).Trim(), tableName, Session.Owner, GetOrderBy());
            else query = string.Format(query, fieldName.Substring(0, fieldName.Length - 1).Trim(), tableName, Session.Owner, GetOrderBy(), "{0}");
            return query;
        }

        private string GetByCodeSql(DataTable dt, string tableName)
        {
            string query = "SELECT {0} FROM {3}.{1} WHERE {2}";
            string fieldName = string.Empty;
            string whereCondition = string.Empty;
            ...
            query = string.Format(query, fieldName.Substring(0, fieldName.Length - 1).Trim(), tableName, whereCondition.Substring(0, whereCondition.Length - 4).Trim(), Session.Owner);
            return query;
        }

        private string GetByCodeSqlUpdVer(DataTable dt, string tableName)
        {
            string query = "SELECT {0} FROM {3}.{1} WHERE {2}";
            string fieldName = string.Empty;
            string whereCondition = string.Empty;
        ...
        private void SetAllKeys(ref string declarationParametersKeys, ref string listParameterKeys, ref string parametersKeysWithOutType, ref string isNullKeys, ref string parameters)
        {
            foreach (KeyValuePair<string, KeyColumn> kvp in Session.Keys)
            {
                ...
                {
                    //SetDeclarationKeys
                    declarationParametersKeys = string.Concat(declarationParametersKeys, kvp.Value.Type, " ", kvp.Key, ",");
                    //SetParametersKeys
                    ...
                    parameters = string.Format("{0}\t\t\tPgSqlParameter parameter{1} = new PgSqlParameter(\"{1}\", DbType.{2}); \n", parameters, GetPublic(kvp.Key), dbType);
                    parameters = string.Concat(parameters, "\t\t\tDataHelperPostGres.ManageParameter(ref parameter", GetPublic(kvp.Key), ", ", GetPublic(kvp.Key), "); \n");
                }
            }
        ...
        private void GetParametersListAll(DataTable dt, ref string parameterList, ref string parameterListDelete, ref string parameterListClone,ref string parameters, Provider provider)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (!dr["Name"].ToString().StartsWith("VERSION_"))
                {
                    ...
                    dbType = dbType.Replace("?", string.Empty);

                    if (dbType.Equals("Bool"))
                    ...
                    if (Session.TableKey == dr["Name"].ToString())
                    {
                        if (provider == Provider.SqlBase)
                        {
                            ...
                            parameters = string.Concat(parameters, "\t\t\t\t\tDataHelperAccess.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", GetLastId(\"", GetPublic(dr["Name"].ToString()), "\")); \n");
                            parameters = string.Concat(parameters, "\t\t\t\telse DataHelperPostGres.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", ", GetPublic(dr["Name"].ToString()), "); \n");
                        }
                        else
                    ...
                    else
                    {
                        if (dr["Name"].ToString().Equals("N_UPD_VER"))
                        {
                            ...
                            parameters = string.Concat(parameters, "\t\t\t\t\tDataHelperAccess.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", GetLastUpdVer()); \n");
                            parameters = string.Concat(parameters, "\t\t\t\telse DataHelperPostGres.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", 1); \n");
                        }
                        else
                            parameters = string.Concat(parameters, "\t\t\t\tDataHelperPostGres.ManageParameter(ref parameter", GetPublic(dr["Name"].ToString()), ", ", GetPublic(dr["Name"].ToString()), "); \n");
                    }
        ...
        private Dictionary<string, KeyColumn> GetKeys(string tableName)
        {
            ...
            string query = string.Format("SELECT key_column_usage.column_name as column_name  FROM  information_schema.table_constraints,information_schema.key_column_usage WHERE table_constraints.constraint_schema = key_column_usage.constraint_schema AND table_constraints.constraint_name = key_column_usage.constraint_name AND table_constraints.constraint_catalog = key_column_usage.constraint_catalog AND table_constraints.table_name='{0}'", tableName);
            DataTable dt = SqlHelperPostGres.ExecuteDataTable(Session.ConnectionString, CommandType.Text, query, null);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                DataTable dtColumn = GetDatatablePostGres.GetSchema(tableName, column.Name);
                if (dtColumn.Rows.Count > 0)
                {
                    column.Type = dtColumn.Rows[0]["Type"].ToString();
                    column.Size = 0;
                    column.NetType = column.Type;
        ...
        public static string GetAccessType(string name, string tstrSqlType)
        {
    ...
    //public class KeyColumnCollection : List<KeyColumn>
    //{
    //}
